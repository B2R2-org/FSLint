...
type DotGetTests() =
  ...
  member _.``[DotGet] Chained property access``() =
    let code =
      """let x = obj.Property.SubProperty.Value"""
    linterForFs.Lint(Constants.FakeFsPath, code)

  [<TestMethod>]
  member _.``[DotGet] In array literal``() =
    let code =
      """let arr = [ obj.Prop1; obj.Prop2; obj.Prop3 ]"""
    linterForFs.Lint(Constants.FakeFsPath, code)

  [<TestMethod>]
  member _.``[DotGet] In tuple``() =
    let code =
      """let t = (obj1.Prop, obj2.Prop, obj3.Prop)"""
    linterForFs.Lint(Constants.FakeFsPath, code)
